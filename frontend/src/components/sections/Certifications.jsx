    ...
    <section id="certifications" className="py-24 px-6 md:px-12 max-w-7xl mx-auto relative z-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {certsData.map((cert, idx) => (
          <div key={`cert-${idx}`} className="glass-card rounded-3xl p-1 shadow-xl hover:-translate-y-2 transition-transform duration-500 transform-gpu overflow-hidden relative group">
            ...
            <div className="bg-surface-container-lowest/80 backdrop-blur-xl w-full h-full rounded-[1.35rem] p-6 lg:p-8 flex flex-col relative z-10 border border-outline-variant/10">
              ...
              <div className="flex justify-between items-start mb-8">
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${cert.color} flex items-center justify-center border border-white/5 shadow-inner group-hover:scale-110 transition-transform duration-500 transform-gpu`}>
            ...
            <div className={`absolute -bottom-20 -right-20 w-48 h-48 bg-gradient-to-br ${cert.color} blur-[60px] opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none rounded-full transform-gpu`}></div>
          </div>
        ))}
      </div>
