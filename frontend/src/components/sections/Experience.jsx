    ...
    <section id="experience" className="py-24 px-6 md:px-12 max-w-5xl mx-auto relative z-10">
      ...
      <div className="relative border-l-2 border-outline-variant/20 ml-4 md:ml-8 pl-8 md:pl-16 space-y-16">
        {experienceData.map((exp, idx) => (
          <div key={`exp-${idx}`} className="relative group">
            {/* Timeline Node */}
            <div className="absolute -left-[41px] md:-left-[73px] top-0 w-8 h-8 rounded-full bg-surface-container-highest border-4 border-background flex items-center justify-center group-hover:scale-125 transition-transform duration-500 transform-gpu z-10">
            ...
            {/* Content Card */}
            <div className={`bg-gradient-to-br ${exp.color} to-surface-container-low/20 backdrop-blur-md border border-outline-variant/20 p-6 md:p-10 rounded-3xl shadow-xl hover:shadow-[0_20px_40px_rgba(0,0,0,0.3)] transition-all duration-500 transform-gpu hover:-translate-y-2`}>
